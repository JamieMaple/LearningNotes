...
TextQuery::TextQuery(std::ifstream& in): content(new QueryResult::text_group_type) {
    string line, word;
    ...
    while (std::getline(in, line)) {
        ...
        while (in_str >> word) {
            if (word_map.find(word) == word_map.end()) {
                word_map[word].reset(new QueryResult::line_num_group_type);
            }
...
QueryResult TextQuery::query(const string& word) const {
    ...
    if (it == word_map.end()) {
        static auto no_data = std::make_shared<QueryResult::line_num_group_type>();
